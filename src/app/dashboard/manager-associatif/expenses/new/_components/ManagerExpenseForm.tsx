...
export function ManagerExpenseForm() {
  const router = useRouter()
  const [state, formAction, isPending] = useActionState(createExpense, initialState)

  useEffect(() => {
    if (state.success) {
      router.push('/dashboard/manager-associatif/expenses')
    }
    ...
    <form action={formAction} className="space-y-5">
      ...
      <div>
        ...
        <textarea
          name="description"
          rows={3}
          placeholder="Ex: Déplacement match aller U15 — 3 voitures"
          maxLength={500}
      ...
      <div>
        ...
        <p className="text-xs mt-1" style={{ color: '#8e8a9c' }}>
          Lien vers un justificatif en ligne (Google Drive, Dropbox…)
        </p>
      </div>
      ...
      <div className="flex items-center gap-3 pt-2">
        ...
        >
          {isPending ? 'Enregistrement…' : 'Soumettre la dépense'}
        </button>
        <a
